...
const DurationComparisonChart: React.FC<DurationComparisonChartProps> = ({ routes }) => {
    // Transform routes data for chart
    const chartData = routes.map(route => ({
        name: route.type,
        duration: route.durationMinutes,
        hours: Math.floor(route.durationMinutes / 60),
    ...
    return (
        <div className="bg-slate-900/40 border border-white/10 rounded-3xl p-6 backdrop-blur-xl">
            <h3 className="text-xl font-black text-white mb-4">Duration Comparison</h3>
            <ResponsiveContainer width="100%" height={300}>
                ...
                >
                    ...
                    <XAxis
                        ...
                        label={{ value: 'Minutes', position: 'insideBottom', offset: -5, fill: '#94a3b8' }}
                    />
                    <YAxis
                        type="category"
                        dataKey="name"
